...
type Registration = { id: number; referenceCode: string; participant: string; email: string; eventId: number; event: string; attendeeId: number; date: string; status: string; checkIn: string };
...
export function RegistrationsPage() {
  ...
  return <div className="mx-auto max-w-[1600px] space-y-5 p-4 sm:p-6">
    ...
    <Dialog open={Boolean(selected)} onOpenChange={(open) => { if (!open) setSelected(undefined); }}><DialogContent><DialogHeader><DialogTitle>Registration details</DialogTitle><DialogDescription>{selected?.referenceCode}</DialogDescription></DialogHeader>{selected && <div className="grid grid-cols-2 gap-4 rounded-lg bg-background p-4 text-sm"><Detail label="Participant" value={selected.participant} /><Detail label="Event" value={selected.event} /><Detail label="Email" value={selected.email} /><Detail label="Registered" value={selected.date} /><Detail label="Status" value={selected.status} /><Detail label="Check-in" value={selected.checkIn} /></div>}</DialogContent></Dialog>
