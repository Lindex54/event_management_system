...
import { AlertCircle,ArrowLeft,Calendar,Clock,Lock,MapPin,MessageSquare,Send,Trash2,Unlock,UserMinus,Users } from "lucide-react";
...
export function DiscussionRoomPage({eventId,basePath}:{eventId:number;basePath:string}){
  ...
  const[message,setMessage]=React.useState("");const[error,setError]=React.useState("");const[sending,setSending]=React.useState(false);const[updatingStatus,setUpdatingStatus]=React.useState(false);
  const lastId=React.useRef(0);const lastTyping=React.useRef(0);const endRef=React.useRef<HTMLDivElement>(null);

  const poll=React.useCallback(async(initial=false)=>{try{const[result,typing]=await Promise.all([discussionApi.messages(eventId,initial?0:lastId.current),discussionApi.typingUsers(eventId)]);setDiscussion(result.discussion);setTypingUsers(typing);if(result.discussion.canModerate)void discussionApi.participants(eventId).then(setParticipants).catch(()=>undefined);if(result.messages.length){lastId.current=result.messages.at(-1)!.id;setMessages(current=>initial?result.messages:[...current,...result.messages.filter(next=>!current.some(existing=>existing.id===next.id))]);}setError("");}catch(reason){setError(reason instanceof Error?reason.message:"Unable to load this discussion");}},[eventId]);
  ...
  async function changeStatus(status:"Open"|"Closed"){setUpdatingStatus(true);try{if(status==="Open")await discussionApi.open(eventId);else await discussionApi.close(eventId);setDiscussion(current=>current?{...current,status}:current);setError("");toast.success(status==="Open"?"Discussion opened":"Discussion closed");}catch(reason){toast.error(reason instanceof Error?reason.message:"Unable to update discussion");}finally{setUpdatingStatus(false);}}
  ...
  async function deleteMessage(messageId:number){try{await discussionApi.deleteMessage(eventId,messageId);setMessages(current=>current.filter(item=>item.id!==messageId));toast.success("Message deleted");}catch(reason){toast.error(reason instanceof Error?reason.message:"Unable to delete message");}}
