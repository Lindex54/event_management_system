...
export function SetupAccountForm() {
  ...
  const [invitee, setInvitee] = React.useState<{ name: string; email: string } | null>(null);
  ...
  React.useEffect(() => {
    ...
    void fetch(`${API_BASE_URL}/api/auth/setup-account?token=${encodeURIComponent(token)}`)
      .then((response) => response.json())
      .then((result) => { if (result.success) setInvitee(result.data); })
      .catch(() => undefined)
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok || !result.success) { setError(result.message ?? "Unable to create your password."); return; }
      if (result.pending) { setPendingMessage(result.message); return; }
      toast.success("Password created. Welcome to Evently!");
  ...
  if (checking) {
    return <p className="text-center text-sm text-text-secondary">Verifying your invitation...</p>;
  }
  ...
  if (!token || !invitee) {
    return (
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, ease: "easeOut" }} className="text-center">
        ...
        <h1 className="mt-4 text-2xl font-bold tracking-tight text-text-primary">Invitation link invalid</h1>
        <p className="mt-3 text-sm leading-6 text-text-secondary">This invitation link is invalid, expired, or has already been used. Ask an administrator to send you a new invitation.</p>
        <Button asChild className="mt-6"><Link href="/login">Go to sign in</Link></Button>
      </motion.div>
    );
  }
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, ease: "easeOut" }}>
      ...
      <h1 className="mt-2 text-3xl font-bold tracking-tight text-text-primary sm:text-4xl">Hi {invitee.name}, create your password</h1>
      <p className="mt-3 text-sm leading-6 text-text-secondary">Set a password for {invitee.email} to activate your account.</p>

      <form onSubmit={submit} noValidate className="mt-8 space-y-5">
        <PasswordField id="setup-password" label="New Password" value={password} onChange={(value) => { setPassword(value); setError(""); }} autoComplete="new-password" placeholder="Create a strong password" />
        ...
        <PasswordField id="setup-confirm-password" label="Confirm Password" value={confirmPassword} onChange={(value) => { setConfirmPassword(value); setError(""); }} autoComplete="new-password" placeholder="Repeat your password" />
