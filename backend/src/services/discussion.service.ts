...
interface ScheduleRow extends RowDataPacket { eventId:number; eventDate:string; startTime:string|null; timezone:string; }
...
function controlsActive(row:Pick<AccessRow,"eventDate"|"startTime"|"timezone"|"eventStatus">){return ["Upcoming","Active"].includes(row.eventStatus)&&localTimestamp(row.eventDate,row.startTime)-nowInZone(row.timezone)<=20*60_000;}

async function activateScheduledDiscussions(eventId?:number){
  const [events]=await pool.query<ScheduleRow[]>(`SELECT id eventId,DATE_FORMAT(event_date,'%Y-%m-%d') eventDate,TIME_FORMAT(COALESCE(start_time,'00:00:00'),'%H:%i:%s') startTime,timezone FROM events WHERE deleted_at IS NULL AND status IN ('Upcoming','Active') ${eventId?"AND id=?":""}` ,eventId?[eventId]:[]);
  for(const event of events){if(localTimestamp(event.eventDate,event.startTime)-nowInZone(event.timezone)>20*60_000)continue;await pool.execute(`INSERT INTO event_discussions(event_id,status,opened_at,closed_at) VALUES(?,'Open',CURRENT_TIMESTAMP,NULL) ON DUPLICATE KEY UPDATE status=IF(closed_at IS NULL,'Open',status),opened_at=IF(closed_at IS NULL,COALESCE(opened_at,CURRENT_TIMESTAMP),opened_at)`,[event.eventId]);}
}
...
function result(row:AccessRow,user:DiscussionIdentity){const active=controlsActive(row);return {eventId:row.eventId,eventName:row.eventName,slug:row.slug,eventDate:row.eventDate,startTime:row.startTime,venueName:row.venueName,discussionId:row.discussionId,status:row.status??"Closed",currentUserId:user.userId,isAdministrator:user.isAdministrator,controlsActive:active,canOpen:(user.isAdministrator||Boolean(row.organizerAccess||row.coOrganizerAccess))&&active,canClose:user.isAdministrator||(Boolean(row.organizerAccess||row.coOrganizerAccess)&&active)};}

async function getAccess(eventId:number,user:DiscussionIdentity){
  await activateScheduledDiscussions(eventId);
...
export async function listDiscussions(user:DiscussionIdentity){
  await activateScheduledDiscussions();
