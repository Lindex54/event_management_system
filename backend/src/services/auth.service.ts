...
interface AccountRow extends RowDataPacket { userId: number; username: string | null; passwordHash?: string; failedLoginAttempts: number; lockedAt: Date | null; firstName: string; lastName: string | null; email: string; roles: string; organizerId: number | null; organization: string | null; attendeeId: number | null; }
...
export type AuthOutcome = { result: "OK"; user: UserIdentity } | { result: "LOCKED" } | { result: "INVALID" };
...
const identitySelect = `SELECT u.id AS userId,u.username,u.password_hash AS passwordHash,u.failed_login_attempts AS failedLoginAttempts,u.locked_at AS lockedAt,p.first_name AS firstName,p.last_name AS lastName,p.email,
...
export async function authenticateUser(identifier: string, password: string): Promise<AuthOutcome> {
  ...
  if(!account?.passwordHash) return { result: "INVALID" };
  if(account.lockedAt) return { result: "LOCKED" };
  if(!(await verifyPassword(password,account.passwordHash))){
    const attempts=account.failedLoginAttempts+1;
    if(attempts>=maxFailedLoginAttempts){
      await pool.execute("UPDATE users SET failed_login_attempts=?,locked_at=CURRENT_TIMESTAMP WHERE id=?",[attempts,account.userId]);
      return { result: "LOCKED" };
    }
    await pool.execute("UPDATE users SET failed_login_attempts=? WHERE id=?",[attempts,account.userId]);
    return { result: "INVALID" };
  }
  if(account.failedLoginAttempts>0) await pool.execute("UPDATE users SET failed_login_attempts=0 WHERE id=?",[account.userId]);
  return { result: "OK", user: identity(account) };
}
