...
interface AccountRow extends RowDataPacket { userId: number; username: string | null; passwordHash?: string; firstName: string; lastName: string | null; email: string; roles: string; organizerId: number | null; organization: string | null; }
export interface UserIdentity { id: number; username: string | null; name: string; email: string; roles: string[]; organizerId: number | null; organization: string | null; }
function identity(row: AccountRow): UserIdentity { return { id: row.userId, username: row.username, name: [row.firstName,row.lastName].filter(Boolean).join(" "), email: row.email, roles: row.roles.split(",").filter(Boolean), organizerId: row.organizerId, organization: row.organization }; }
...
const identitySelect = `SELECT u.id AS userId,u.username,u.password_hash AS passwordHash,p.first_name AS firstName,p.last_name AS lastName,p.email,
  GROUP_CONCAT(DISTINCT r.slug ORDER BY r.slug) AS roles,o.id AS organizerId,o.organization
...
export async function authenticateUser(identifier: string, password: string): Promise<UserIdentity | null> {
  const [rows] = await pool.query<AccountRow[]>(`${identitySelect} WHERE (LOWER(p.email)=LOWER(?) OR LOWER(u.username)=LOWER(?)) AND u.status='Active' AND u.deleted_at IS NULL GROUP BY u.id,p.id,o.id LIMIT 1`, [identifier,identifier]);
